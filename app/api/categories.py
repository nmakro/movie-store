...
def list_categories():
    ...
    if genre:
        if genre == "all":
            ...
            categories = Category.query.filter_by(genre=genre).paginate(
                page, per_page, False
            )
        next_url = (
            url_for("api.list_categories", page=categories.next_num, genre=genre)
            if categories.has_next
            else None
        )
        prev_url = (
            url_for("api.list_categories", page=categories.prev_num, genre=genre)
            ...
            (
                {
                    ...
                    "categories": [c.category_dict() for c in categories.items],
                },
                200,
            )
            if categories.items
            else ({"error:": "Category not found!"}, 404)
        )

    else:
        items = Category.query.order_by(Category.genre.desc())
        data, status_code = (
            ({"categories": [c.genre for c in items]}, 200)
            if items
            else ({"error:": "No categories found"}, 404)
        )
    res = jsonify(data)
