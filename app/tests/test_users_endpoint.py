...
class TestRoutes(flask_testing.TestCase):
    ...
    def testListUsersAuthorized(self):
        with self.app.test_client() as c:
            ...
            self.mock_user.query.filter_by.return_value.first.return_value = Mock(
                id=5, username="Bob", **attrs
            )
            r = c.get(
                "/api/v1/users",
                headers={"Authorization": _basic_auth_str("admin", "admin_pass")},
            )
    ...
    def testListUsersAuthorizedNoUsersFound(self):
        with self.app.test_client() as c:
            ...
            u2 = Mock(**attrs)
            self.mock_user.query.order_by.return_value.paginate.return_value = Mock(
                items=[u1, u2]
            )
            r = c.get(
                "/api/v1/users",
    ...
    def testUpdateUsers(self):
        ...
        with self.app.test_client() as c:
            ...
            r = c.patch(
                "/api/v1/users/5",
                headers={"Authorization": _basic_auth_str("Chris", "test_user_pass")},
            )
            self.assertEqual(r.status_code, 401)
        ...
        with self.app.test_client() as c:
            self.mock_user.query.filter_by.return_value.first.side_effect = [
                Mock(id=5, username="Chris"),
                None,
            ]
        ...
        with self.app.test_client() as c:
            ...
            self.assertEqual(
                r.json["message"],
                "Username John is used by another user. Please user another username.",
            )
